...
let AuthService = class AuthService {
    ...
    async signIn(signInDto, response) {
        ...
        if (!(await (0, helper_helpers_1.comparePassword)(signInDto.password, user.password))) {
        ...
        await this.prisma.users.update({
            where: { id: user.id },
            data: {
                accessToken: await (0, helper_helpers_1.hashPassword)(accessToken),
            },
        });
        return {
            email: user.email,
    ...
    async logout(response) {
        var _a;
        const accessToken = (_a = response.req.headers['authorization']) === null || _a === void 0 ? void 0 : _a.split(' ')[1];
        if (!accessToken) {
