...
export function RecordPaymentDialog({ bill }: RecordPaymentDialogProps) {
  const { recordPayment, loading } = useSubcontractorBillsStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await recordPayment(bill.id, Number(amountPaid));
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (open) { setAmountPaid(bill.amountPaid); setError(""); } }}>
      ...
      <DialogContent className="sm:max-w-[380px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Total: {Number(bill.amount).toLocaleString()} {bill.currency}. Status is recomputed automatically.
          </DialogDescription>
        </DialogHeader>
