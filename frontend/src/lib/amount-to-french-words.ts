...
// bottom of the invoice PDF, e.g. amountToFrenchWords(8000) -> "Huit mille
// dirhams et 00 centimes." Centimes are kept as digits, matching how this
// line is actually written on real invoices.
//
// Deliberately duplicated from the backend's identical helper
// (backend/src/client-invoices/helpers/amount-to-french-words.ts) — the
// frontend needs it to live-update the suggestion as the user edits totals
// in the Generate Invoice dialog, and this repo has no shared package between
// the two Node projects. Keep both in sync if either changes.
...
export function amountToFrenchWords(amount: number, currency: "MAD" | "EUR" = "MAD"): string {
